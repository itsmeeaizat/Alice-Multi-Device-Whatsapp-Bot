...
module.exports = {
  command: ['sf', 'sfile', 'sfiledl', 'sfdl'],
  operate: async (context) => {
    const { Alice, XRB, XReaction, AliceCmd, axios, cheerio, command, content, isBan, m, q, quoted, reply, text } = context;

    switch (command) {
case 'sf': case 'sfile': case 'sfiledl': case 'sfdl': {
if (isBan) return XRB()
await XReaction()
if (!text.includes('https://sfile.mobi')) return reply(`• *Example :* ${AliceCmd} https://sfile.mobi/xxxxxxx/`)
