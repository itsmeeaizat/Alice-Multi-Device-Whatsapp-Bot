...
module.exports = {
  ...
  operate: async (context) => {
    ...
    try {
      ...
      await execAsync(`ffmpeg -i ${inputPath} -filter:a "asetrate=44100*2,aresample=44100" ${outputPath}`);
      const buf = fs.readFileSync(outputPath);
      await Alice.sendMessage(m.chat, { audio: buf, mimetype: 'audio/mpeg' }, { quoted: m });
    } catch (err) {
      console.error(err);
      await reply('Gagal memproses audio chipmunk.');
    } finally {
